	...
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "luaSpriteMakeGraphic", function(tag:String, width:Int, height:Int, color:String) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag))
				psychlua.ScriptHandler.modchartSprites.get(tag).makeGraphic(width, height, CoolUtil.colorFromString(color));
		});
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				var cock:ModchartSprite = psychlua.ScriptHandler.modchartSprites.get(tag);
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				...
				var pussy:ModchartSprite = psychlua.ScriptHandler.modchartSprites.get(tag);
		...
		Lua_helper.add_callback(lua, "luaSpritePlayAnimation", function(tag:String, name:String, forced:Bool = false) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				psychlua.ScriptHandler.modchartSprites.get(tag).animation.play(name, forced);
			}
		});
		Lua_helper.add_callback(lua, "setLuaSpriteCamera", function(tag:String, camera:String = '') {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				psychlua.ScriptHandler.modchartSprites.get(tag).cameras = [LuaUtils.cameraFromString(camera)];
				return true;
			}
		...
		Lua_helper.add_callback(lua, "setLuaSpriteScrollFactor", function(tag:String, scrollX:Float, scrollY:Float) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				psychlua.ScriptHandler.modchartSprites.get(tag).scrollFactor.set(scrollX, scrollY);
				return true;
			}
			return false;
		});
		Lua_helper.add_callback(lua, "scaleLuaSprite", function(tag:String, x:Float, y:Float) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				var shit:ModchartSprite = psychlua.ScriptHandler.modchartSprites.get(tag);
				shit.scale.set(x, y);
				shit.updateHitbox();
				return true;
			}
		...
		Lua_helper.add_callback(lua, "getPropertyLuaSprite", function(tag:String, variable:String) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				...
				if(killMe.length > 1) {
					var coverMeInPiss:Dynamic = Reflect.getProperty(psychlua.ScriptHandler.modchartSprites.get(tag), killMe[0]);
					for (i in 1...killMe.length-1) {
						coverMeInPiss = Reflect.getProperty(coverMeInPiss, killMe[i]);
					}
					return Reflect.getProperty(coverMeInPiss, killMe[killMe.length-1]);
				}
				return Reflect.getProperty(psychlua.ScriptHandler.modchartSprites.get(tag), variable);
			}
			return null;
		});
		Lua_helper.add_callback(lua, "setPropertyLuaSprite", function(tag:String, variable:String, value:Dynamic) {
			...
			if(psychlua.ScriptHandler.modchartSprites.exists(tag)) {
				...
				Reflect.setProperty(psychlua.ScriptHandler.modchartSprites.get(tag), variable, value);
				return true;
			}
